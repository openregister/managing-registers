...
RSpec.describe ValidationHelper do
  ...
  describe "get_form_errors" do
    it "returns an error if date is invalid" do
      ...
      expect(data_validator.get_form_errors(params,field_definitions, 'country', 'create', nil).messages).to eql({:start_date=>["foo is not a valid date"]})
    end

    it "returns no errors if date is valid" do
      params = {"start-date"=>"2015-05", "country"=>"xx" }
      expect(data_validator.get_form_errors(params, field_definitions, 'country', 'create', nil).details).to be_empty
    end

    it "returns an error if key is not populated" do
      params = {:country=>'', :register=>"country"}
      expect(data_validator.get_form_errors(params, field_definitions, 'country', 'create', nil).messages).to eql({:country => ["Field Country is required"]})
    end
  end
